...
def signal(df,w):
    ...
    a=r["ATR"]
    tp_mult=2.5
    return {"side":side,"entry":p,"sl":p-a*1.5,"tp":p+a*tp_mult,"score":s}
...
async def scan(ctx):
    ...
    for s in CRYPTOS:
        ...
        if cd and cd[0]>time.time():
            continue
        df=enrich(await fetch(s,"4h",600))
        if df.empty:
            continue
...
async def track(ctx):
    ...
    for t in c.execute("SELECT id,symbol,qty,entry,sl,tp FROM trades WHERE status='OPEN'").fetchall():
        df=enrich(await fetch(t[1],"15m",200))
        if df.empty:
            continue
        p=df.iloc[-1]["c"]
...
async def backtest_cmd(update,ctx):
    ...
    if sym not in CRYPTOS:
        ...
        return
    df=enrich(await fetch(sym,"4h",3000))
    wins=losses=0
    pnl=0
...
async def analyze(update,ctx):
    ...
    await q.answer()
    df=enrich(await fetch(q.data,"4h",600))
